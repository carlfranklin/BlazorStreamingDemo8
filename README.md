...
Create a new console app called **Streaming**  and replace *Program.cs* with the following:
...
##### Create a Global WebAssembly Blazor Web App called *SignalRStreamingDemo* 

![image-20240429063725310](images/image-20240429063725310.png)

Make sure to check the following options:

![image-20240429063748703](images/image-20240429063748703.png)

Add the latest version of the following package to the client project's .csproj file:

```xml
<PackageReference Include="Microsoft.AspNetCore.SignalR.Client" Version="8.0.4" />
```

SignalR server-side code (Hubs) is built into ASP.NET Servers, but we need this library in order to call into the hub from Blazor.

Now add these global using statements to the top the client project's *Program.cs* file:
...
Open the server project's *Program.cs* file 

Add the following just before the line `var app = builder.Build();`
...
Add the following before the line `app.Run();`
...
This hub was adapted from the documentation at https://learn.microsoft.com/en-us/aspnet/core/signalr/streaming?view=aspnetcore-8.0
...
Before we run any Blazor code, let's add this statement to the client project's *_Imports.razor*
...
<head>
    ...
    <link rel="stylesheet" href="SignalRStreamingDemo.styles.css" />
...
Now, change the contents of *Home.razor* to the following:
...
We're going to start with an existing project, ***BlazorGrpcDemo***, which compares the performance of downloading 5000 records (don't do this at home, kids) using both a REST endpoint and a gRPC service.

I've copied the finished project into the *!Completed Projects\12-Streaming* folder.
...
Replace *ApiService.cs* with the following:
...
This would be a good time to replace the contents of *Home.razor*:
...
![image-20240429065658288](images/image-20240429065658288.png)
...
![image-20240429065752589](images/image-20240429065752589.png)
...
![image-20240429065821370](images/image-20240429065821370.png)

46 ms after I pressed the button, the list filled up with records. About 5 seconds later the last record was retrieved. That's a great improvement, UI-wise, over waiting until the entire set has been downloaded.

Now let's compare this to the gRPC stream. Here's what I get:

![image-20240429065859112](images/image-20240429065859112.png)

Now that's fast! The records filled the screen in only 18ms! 
...
gRPC is clearly the performance leader. It also requires a bit less ceremony than creating standard API Controllers.
